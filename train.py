...
def train_it(filename,seed1,seed2,iteration,mode):
    import tensorflow as tf
    #setting 
    seed(seed1)
   ...
   # training_iters = iteration
    ...
    tppath="./models/models_predict{}_{}/train_all_for_predict.ckpt".format(filename[0:20],iteration)
    #predict_save_file='./outcome/models_{}_{}/{}_AUC{}.csv'.format(filename[0:20],iteration,iteration,aucvalue)
    mode_save_file="./models/models_predict{}_{}".format(filename[0:20],iteration)
   ## predict_file='
    weights = {
    # (28, 128)
        ...
        with tf.Session() as sess:
             ...
             while step * batch_size < epo*save_point:
                ...
                if step % 400 == 0:
                   ...
                   #save per200 step
                   saver.save(sess=sess, save_path="./models/models_predict{}_{}/train_all_for_predict.ckpt".format(filename[0:-4],epo), global_step=step+1)
              ...
              with tf.Session() as sess:
                 ...
                 while step * batch_size < training_iters:
                     ...
                     if step % 200 == 0:
                         ...
                         print(sess.run(accuracy, feed_dict={x: x_test_batch,y: y_test_batch,}))
                         saver.save(sess=sess, save_path="./models_{}_{}/train_for_validate.ckpt".format(filename[0:20],iteration), global_step=step+1)
                     if step % 500 == 0:
                          print('-------------monitor-----------------------------------line-------------------')
                     step += 1
    if mode=='validation':
     ...
     with tf.Session() as sess:
        ...
        yreal=[]
        try:
          module_file = tf.train.latest_checkpoint('./models_{}{}'.format(filename[0:-4],iteration))
    ...
    #list_pairs=[]
       ...
       with tf.Session() as sess:
         ...
         if predict_mode=='file':
             ...
             module_file = tf.train.latest_checkpoint("./models/models_predict{}_{}".format(filename[0:-4],epo))#
             #epo+=50
             saver.restore(sess,module_file)
             print('succssfully load the prediction model')
             y_score=[]
             ...
             print(ori)
             ori.to_csv('./outcome/{}_test_results_{}_auc{}.csv'.format(filename[0:-4],iteration,roc_auc),index=None)
             return roc_auc
   #np.save('LSTM_y_label',test_label)
...
def train_vali(i):
 ...
 train_it(filename,0,0,i*1000000,'train_some_dataset_for validation')#train_some_dataset_for validation
 tf.reset_default_graph()   
 train_it(filename,0,0,500000,'validation')#validation       
 tf.reset_default_graph()
def train_pre(file,i):
 tf.reset_default_graph() 
 train_it(file,0,0,i,'train_all_dataset for the predict')#train_some_dataset_for validation
 tf.reset_default_graph() 
 auc=train_it(file,0,0,i,'predict')
 tf.reset_default_graph() 
 return auc
def train_pre_fivefold(filename):
   for j in range(0,3):
    list_result=[]
    for i in range(1,6):
      ...
      print(five_name)
      epoch=500
      auc=train_pre(five_name,epoch)
      list_result.append({'iter':j,'epoch':epoch,'auc':auc})
      lis=pd.DataFrame(list_result)
      lis.to_csv('summary_{}_{}_{}.csv'.format(filename[0:-4],j,epoch),index=None)
train_pre_fivefold(filename)