...
def embedding2dataset(mirnafile,genefile,randomseed,dataset,outputname):
           ...
           if test_mode== 'On':
            ...
            X_train, X_test, y_train, y_test = train_test_split(X, Y,test_size=0.1, random_state=0)#5折交叉验证
...
def five_fold_crossvalidation(outputname,seq,ex):#feed X into theses return five 
   ...
   # kf = 
    ...
    #KF=KFold(n_splits=5)  #建立5折交叉验证方法  查一下KFold函数的参数 establish KFOLD
    count=0
...
#embedding2dataset('mirna_geo_1948.csv','BIOGRID-ALL-4.1.190(no repeate)18875-role2vec-embedding.csv',0,'hsa+neg.csv','G+Gnew.csv')#embedding-mirna#embdding-gene#randomseed#file#outputname
for i in range(0,3):
 five_fold_crossvalidation('G+Gnew.csv',i,1)#seq of divi/export to csv（1 tru,0 false) 